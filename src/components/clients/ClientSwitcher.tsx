            ...
            <div className="relative px-3 py-2">
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-xs text-violet-600 dark:text-violet-300 font-medium">Cliente ativo</p>
                        <p className="text-sm text-foreground font-semibold truncate">
                            {activeTenant?.name || 'Selecionar...'}
                        </p>
                    </div>
                    <ChevronDown className={cn('w-4 h-4 text-violet-500 dark:text-violet-300 transition-transform', open && 'rotate-180')} />
                </button>

                {open && (
                    <div className="absolute left-3 right-3 top-full mt-1 bg-popover border border-border rounded-xl shadow-xl z-50 overflow-hidden">
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeTenant?.id === client.id
                                            ? 'bg-violet-600/20 text-violet-700 dark:bg-violet-600/30 dark:text-violet-200'
                                            : 'text-muted-foreground hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-foreground'
                                    )}
                                >
                                    <div className={cn(
                                        'w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0',
                                        client.slug === 'fsa-launch-lab' ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-600 dark:bg-slate-600 dark:text-slate-200'
                                    )}>
                                        {client.name.charAt(0).toUpperCase()}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-medium truncate">{client.name}</p>
                                        <p className="text-xs text-muted-foreground capitalize">{client.plan}</p>
                                    </div>
                                    {activeTenant?.id === client.id && (
                        ...
                        <div className="border-t border-border p-1.5">
                            <button
                                ...
                                className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-all text-sm font-medium"
                            >
